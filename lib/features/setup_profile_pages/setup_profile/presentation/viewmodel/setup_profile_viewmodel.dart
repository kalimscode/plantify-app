...
class SetupProfileViewModel extends StateNotifier<SetupProfileState> {
  ...
  void setPhone(String value) =>
      state = state.copyWith(profile: state.profile.copyWith(phone: value));
  ...
  Future<void> loadProfile() async {
    ...
    try {
      ...
      debugPrint('🔑 Session email: $email');
      if (email == null) return;

      final profile = await getProfileUseCase!.call(email);
      debugPrint('👤 Loaded profile in VM: fullName="${profile?.fullName}"');
